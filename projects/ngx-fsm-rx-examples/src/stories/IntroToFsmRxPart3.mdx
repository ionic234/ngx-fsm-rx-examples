...
<article>
  ...
  However the dreaded specter of "feature creep" becomes a lot less daunting when using FSMs.</p>

  <p>One of the key benefits of FSMs is their compartmentalised structure of module-like states with strictly defined transitions, which make adding, removing and reordering functionality simple. 
  In this example, we imagine a scenario where we are given the last-minute requirement of adding a right turn light into our Simple Traffic Light Component.
  Management would like this right green arrow to be on for the last 3 seconds of the <b>"stop"</b> light and for all of the <b>"go"</b> light. </p> 

  <div class="breakout-box examples">
    <p>The complete code for the following ["Traffic Light Right Turn"](?path=/story/examples-2-traffic-light-right-turn--default) <br/> example story can be found in the folder <br/><b>"/projects/examples/src/stories/traffic-light-right-turn"</b>.</p>
  </div>

  <h3>Adding a New State</h3>

  <p>By analyzing the requirements we can identify three changes that need to be made.</p>
  
  <ol>
    <li>Create a new state where both the stop and the right turn signals are active at the same time which lasts for three seconds, called <b>"stopWithRightTurn"</b>.</li>
    <li>Reduce the duration of the stop state by three seconds to account for the time spent in the new <b>"stopWithRightTurn"</b>.</li>
  ...
  <p>To set up the new state first it required adding to the Traffic Lights state string union:</p>
  
  ```typescript
  type TrafficLightStates = "go" | "prepareToStop" | "stop" | "stopWithRightTurn";
  ```
  
  <p>Next the <b>"stopWithRightTurn"</b> is added to the <b>TrafficLightCanLeaveToMap</b> with the <b>"stop"</b> state updated to transition to the new <b>"stopWithRightTurn"</b> state which in turn is set to transition to the <b>"go"</b> state.</p>
  ...
  What's more, if the <code>changeState</code> and <code>updateState</code> function calls are typed to the expected current state, changes to the <code>CanLeaveToMap</code> that invalidate these calls will be highlighted in the IDE, making even complex code simple to maintain.</p>

  <h3>Updating the Data</h3>

  <p><b>FsmRx</b> also makes opdating data easy.   
  ...
  <p>The first step in adding the right turn arrow to the template is to create an <b>HTML</b> element to hold the right turn arrow. 
  ...
  In angular the <code>ngClass</code> directive supports compound conditionals so we can easily add this functionality to the relevant <code>ngClass</code> expressions using the <code>||</code>(or) operator.</p>
...
<hr/>
<p>So far we have created and updated FSMs that work as closed systems. However, real work applications don't exist in a vacuum. 
In ["Part 4: Reacting to user input"](?path=/docs/4-reacting-to-user-input--docs) we will explore how FsmRx handles user interactions by adding a pedestrian crossing button to our traffic light component.</p>
</article>
