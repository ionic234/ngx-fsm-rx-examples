...
<article>
  ...
  <p>During component development it can often be useful to execute the code you are working on for validation, debugging and experimentation purposes. 
  ...
  <p>To speed up the development FsmRx has been designed to work with <a href = "https://storybook.js.org/docs/get-started/why-storybook" target="_blank">Storybook</a>, a frontend playground built for the rapid testing of components in isolation.</p>

  <div class="breakout-box examples">
    <p>The complete code for the following ["Traffic Light Overridable"](?path=/story/examples-6-traffic-light-overridable--default) <br/> example story can be found in the folder <br/><b>"/projects/examples/src/stories/traffic-light-overridable"</b>.</p>
  </div>

  <h3>Defining a Storybook Story</h3>

  <p>To display a component in <b>Storybook</b> we create a special config file known as a <b>Story</b> with the extension ".stories.ts".
  These stories are written in <b>Component Story Format (CSF)</b> and allow us to configure components for use within the <b>Storybook</b> playground, while also defining metadata such as its title and any arguments or inputs it accepts.</p>
  ...
  ```
  <p>The <code>Meta</code> property allows us to configure the controls, interactions and default arguments/inputs of a given component class, while the <code>Story</code> property allows us to configure an individual instance of a component.</p>
  ...
  <p>We can add a <code>decorators</code> section to our stories <code>Meta</code> to wrap components with additional functionality or styling.
  Inside <code>decorators</code> we can define a <code>moduleMetadata</code> parameter for importing external modules and declaring child component dependencies.
  This <code>moduleMetadata</code> is analogous to <b>Angulars</b> <code>NgModule</code> declarations.</p>
  ...
  <p>We can also define a decorator to wrap our component in additional <b>HTML</b> by defining a <code>componentWrapperDecorator</code> function.</p>
  ...
  <p><b>FsmRx</b> contains a helpful wrapper component called <code>FsmRxDebugSetComponent</code>. 
  ...
  <figure>
    <div>
      ...
      <figcaption>The state diagram and debug log provided by wrapping the <code>TrafficLightOverridableComponent</code> in an <code>FsmRxDebugSetComponent</code>.</figcaption>
    </div>
  </figure>

  <p>To configure a story to use the <code>FsmRxDebugSetComponent</code> it must be added, along with its children, to the <code>Meta.decorators.moduleMetadata</code> property:</p>
  ...
  <p>To ensure proper rendering of the <code>FsmRxDebugSetComponent</code> it's essential to update the target <b>FsmRxComponents</b> element to include the <code>#fsmRxComponent</code> template reference variable. 
  This reference is necessary for the <code>FsmRxDebugSetComponent</code> to interface with the target <b>FsmRxComponent</b> and bind to its outputs. </p>
  ...
  This allows us to modify it with any string transformation function. </p>
  ...
  ```
  <p>The <code>FsmRxDebugSetComponent</code> supports the input parameter <code>debugLogKeys</code>, which is an array of state data properties to include in the <b>"Transition Data"</b> column of the debug log and allows for easy configuration of the data to be displayed.
  ...
  Many of these options affect what data is logged and emitted making them useful for debugging with the <code>FsmRxDebugSetComponent</code>.</p> 
   
  <p>The following table documents all of the available configuration options:</p> 
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>Whether output transition rejections are logged to the browser's console.</td>
        <td>true</td>
        <td>false</td>
        <td>Only writable in dev mode.</td>
      </tr>
  ...
  <p>A useful control for debugging <code>FsmRxComponents</code> that <b>Storybook</b> fails to discover is the <code>fsmConfig</code> input which can be configured as follows:</p>
  ...
  <p>Once installed via the command <code>npm install storybook-addon-deep-controls</code> the plugin is enabled using the <code>Meta.parameters</code> object.</p>
  ...
  <h3>Configuring Controls uUsing <code>argTypes</code></h3>
  ...
  This could be because changing them may break functionality or have no discernable effect after initialization.
  For the <code>TrafficLightOverridableComponent</code> <b>Storybook</b> identifies the following unwanted controls</p>

  <figure>
    <div>
      <img src={unwantedControls} alt="Unwanted controls for the TrafficLightOverridableComponent" style={{ paddingBottom: '57px' }} />
      <figcaption>The unwanted controls <b>Storybook</b> generates for the <code>TrafficLightOverridableComponent</code>.</figcaption>
    </div>
  </figure>
  ...
  <p><i>Note: The use of <code>stateOverride</code> in production is not supported, as it can cause unexpected behavior, and is only intended for debugging purposes. 
  ...
  <p>An override state can be added to the <code>TrafficLightOverridableComponent</code> using the <code>Meta</code>.</p>
  ...
  <p>This will add default controls that allow us to change the state of the FSM to any other required state.
  As with other <code>controls</code>, <code>deepControls</code> and the <code>argTypes</code> object can be used to customize them as required:</p>
  ...
  <figure>
    <div>
      ...
      <figcaption>Custom controls that allows for overwriting of the <code>TrafficLightOverridableComponent</code> state data.</figcaption>
    </div>
  </figure>
  ...
  <p>The <code>TrafficLightOverridableComponent</code> defines an <code>onEnter</code> function, shared between each of its states, called <code>handleEnterState</code>.
  ...
  <p>Applying an override to the <code>TrafficLightOverridableComponent</code> bypasses this function. 
  ...
  <p>In the preceding <code>onOverride</code> function for the <code>TrafficLightOverridableComponent</code> the <code>originalStateInfo</code> is used to retrieve the ID of the original states ongoing timeout for clearing, 
  while the <code>overridingStateInfo</code> is used to schedule the transition to the next state.</p>

  <p><i>Note: For an <code>onOverride</code> function defined in a stories <code>Meta</code> to have access to the <code>FsmRxComponents</code> methods and properties it must be provided a reference to its <code>This</code> object, which can be achieved using parameter annotation.</i></p>
   
  <h3>Canceling RxJS Subscriptions when Overriding</h3>
  ...
  <div class="breakout-box examples">
    <p>The complete code for the following ["Traffic Light Overridable Alternate"](?path=/story/examples-7-traffic-light-overridable-alternate--default) <br/> example story can be found in the folder <br/><b>"/projects/examples/src/stories/traffic-light-overridable-alternate"</b>.</p>
  </div>
  ...
  It is also updated at the end of the transition after the callbacks have been executed.</i></p>
  ...
  <p>The <code>UserLoginComponent</code> example login form contains an <code>errorMessage</code> property that is only defined in the state data if the user inputs an invalid username and password combination.</p>

  <div class="breakout-box examples">
    <p>The complete code for the following ["User Login"](?path=/story/examples-8-user-login--static) <br/> example story can be found in the folder <br/><b>"/projects/examples/src/stories/user-login"</b>.</p>
  </div>
  ...
  <p>When applied to the <code>UserLoginComponent</code> story this creates two entries under <b>"User Login"</b> called <b>"Static"</b> and <b>"Functional"</b>.</p>
  ...
  In the event of a failed transition, developers can get details about the failure and perform additional side effects, like bug reporting, via the automatically invoked <code>onTransitionRejected</code> function.</p>
  ...
  <p>By default this function is empty but can be overridden like so:</p>
  ...
  <ul>
    ...
    <li><code>illegal_update_FSMInit</code>: An attempt was made to update the FSMInit State. </li>
    ...
    <li><code>terminated_by_enter_callback</code>: The onEnter callback returned false, terminating the transition.</li>
    <li><code>terminated_by_leave_callback</code>: The onLeave callback returned false, terminating the transition.</li>
    <li><code>terminated_by_update_callback</code>: The onUpdate callback returned false, terminating the update. </li>
  ...
  <div className="breakout-box">
    ...
    As <b>FsmRx</b> is built upon <b>RxJS</b> we can use its <code>TestScheduler</code> utility test to write simple robust asynchronous unit tests.</p>
          
    <p>The <code>TestScheduler</code> enables assessment of the behavior of the<code>observables</code> in the system over time by scheduling events in a simulated timeline, running this simulation, and then comparing the values emitted with those anticipated in a <code>Marble Diagram</code>.</p>

    <p><i>The following example can be found at:<br/> <b>"/projects/examples/src/stories/traffic-light-simple/traffic-light-simple.spec.ts"</b> </i></p>
    ...
    ``` 
    <p>Tests that utilize the <code>TestScheduler</code> can be though of as having two phases, the <b>schedule</b> phase and the <b>run</b> phase.</p>
    ...
    <p>When testing with <code>FsmRx</code>, we usually test events that cause the <code>OnChange</code> or <code>OnUpdate</code> functions to be called. 
    ...
    <p>In the <b>Run</b> phase, the scheduled events are executed in a virtualized time environment using the <code>TestScheduler.run</code> function.
    ...
    When testing <code>FsmRx</code> we use the <code>expectObservable</code> utility method to subscribe to our SUT's <code>stateData$</code> observable stream and assert against the values it emits using the returned <code>toBe</code> operator.
    ...
    Emitted data is represented by a single alphanumeric value, while time progression can be described using either dashes "-" or millisecond values e.g. "10ms".
    ...
    <p>The following example shows a test to assert that the <code>TrafficLightSimpleComponent</code> (demonstrated in ["Part 2: Creating a simple traffic light component with FsmRx"](?path=/docs/2-creating-a-simple-traffic-light-component-with-fsmrx--docs))
    ...
    <p>To solve this we can delay the execution of <code>ngAfterViewInit</code> until after we have subscribed to our SUT observable, by moving the <code>fixture.detectChanges();</code> out of the <code>beforeEach</code> function and into a <code>testScheduler</code> scheduled event.
    This event can be scheduled at any time we desire, however, if we wish to capture the initial <b>"FSMInit"</b> state it should not scheduled at 0.</p>
    ...
    waiting at each state for 1ms, 7seconds, 3seconds, 10seconds and 1ms. 
