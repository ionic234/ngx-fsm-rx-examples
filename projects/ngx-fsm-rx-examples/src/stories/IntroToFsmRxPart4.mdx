...
<article>
  ...
  <ul>
    ...
    <li><b>CrossingControlBoxComponent</b>: A new <code>FsmRxComponent</code> that consists of a request crossing button, a light that indicates that this request has been received and a display that indicates when it is safe to cross.</li>
  ...
  <figure>
    <div>
      <img src={CrossingControlBoxComponent} alt=" Crossing Control Component and State Diagram." style={{ paddingBottom: '60px' }} />
  ...
  <p>The <code>CrossingControlBoxComponent</code> contains a button element called <b>walk-button</b> which can be used to initiate the pedestrian crossing procedure. 
  ...
  <p>During the pedestrian crossing procedure it is vital that the states of the <code>CrossingControlBoxComponent</code> and <code>TrafficLightRequestCrossingComponent</code> remain synchronized. 
  To ensure this happens, the <code>TrafficLightRequestCrossingComponent</code> acts as the "controller" with the responsibility of informing the "delegate" <code>CrossingControlBoxComponent</code> of the state it should be in.
  ...
  <p>These composite states describe both the visual state of the <code>TrafficLightRequestCrossingComponent</code> (<b>"stop"</b>) and the instruction sent to the <code>CrossingControlBoxComponent</code>.
  Entering <b>"stop_startWalking"</b> emits a signal telling the <code>CrossingControlBoxComponent</code> to enter its <b>"startWalking"</b> state while entering <b>"stop_startWalking"</b> signals <b>"finishWalking"</b> and leaving <b>"stop_startWalking"</b> signals <b>"doNotWalk"</b>.</p>
  ...
  <p>When the <code>CrossingControlBoxComponent</code> emits the <code>outputWalkButtonClicked</code> event, it is first captured by the <code>RequestCrossingAssemblyComponent</code> then passed to the <code>TrafficLightRequestCrossingComponent</code> via its <code>walkButtonEvent</code> <code>@Input</code>.
  ...
  ```
  <p>As previously discussed traversing the <b>"stop_startWalking"</b> and <b>"stop_finishWalking"</b> path will cause the <code>TrafficLightRequestCrossingComponent</code> to emit an event triggering the <code>CrossingControlBoxComponent</code> to transition to the supplied state.
  As with previous events, this <code>outputCrossingControlBoxState</code> emission is first captured by the <code>RequestCrossingAssemblyComponent</code> before being relayed to the <code>CrossingControlBoxComponent</code>. 
  ...
  <p>The <code>interval</code> that handles the flashing of the crossing indicator light requires unsubscribing on both the destruction of the <code>CrossingControlBoxComponent</code>, using <code>takeUntil(this.destroy$)</code>, and when transitioning to the next state, using <code>takeUntil(this.nextChangeStateTransition$)</code>.</p>
