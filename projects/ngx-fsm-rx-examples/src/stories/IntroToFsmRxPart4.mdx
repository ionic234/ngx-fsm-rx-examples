...
import crossingControlComponent from "./assets/part4/crossing-control-component.png"
...
<article>
  ...
  <ul>
    <li><b>TrafficLightRequestCrossingComponent</b>: An updated <code>TrafficLightSimpleComponent</code> with additional functionality to handle sequencing of the pedestrian crossing procedure.</li>
    <li><b>CrossingControlComponent</b>: A new <code>FsmRxComponent</code> that consists of a request crossing button, a light that indicates that this request has been received and a display that indicates when it is safe to cross. </li>
    <li><b>RequestCrossingAssemblyComponent</b>: A standard Angular component that facilitates communication between the <b>CrossingControlComponent</b> and the <b>TrafficLightRequestCrossingComponent</b></li>
  </ul>
  ...
  <p>This system follows the controller-delegate pattern where the <b>TrafficLightRequestCrossingComponent</b>(controller) manages the state of the <b>CrossingControlComponent</b>(delegate) duriung the pedestrian crossing procedure.</p>

  <div class="breakout-box examples">
    <p>The complete code for the following ["Traffic Light Request Crossing"](?path=/story/examples-4-traffic-light-request-crossing--default) <br/> example story can be found in the folder <br/><b>"/projects/examples/src/stories/traffic-light-request-crossing"</b>.</p>
  </div>
  ...
  <ol>
    <li>When a pedestrian presses the button on the control box it signals the traffic light to initiate the pedestrian crossing procedure during the next <b>"stop</b> state. 
    This also causes the control box to disable this button and turn on the request crossing indicator light.</li>
    <li>Once the traffic light turns red, it sends a signal to the control box that it is now safe for the pedestrian to cross the road, prompting it to illuminate its <b>"walk"</b> light and to turn off its request indicator light.</li>
    <li>After a predetermined duration set by the traffic light, another signal is sent to the control box. 
    This signal informs the control box that it is time for the pedestrians to complete their crossing, which is communicated by the flashing of the <b>"walk"</b> light. During this step, the request button is also re-enabled.</li>
  ...
  <p>The <code>CrossingControlBoxComponent</code> allows the user to press a button and request the ability to cross the road the next time the traffic light turns red.
  ...
  <figure>
    <div>
      <img src={crossingControlComponent} alt=" Crossing Control Component and State Diagram." style={{ paddingBottom: '60px' }} />
      <figcaption>The rendered CrossingControlComponent and its state diagram.</figcaption>
    </div>
  </figure>

  <h3>The State Data</h3>

  <p>From the specification of the crossing control box component the following four necessary variables can be identified.</p>
  ...
  <ol>
    ...
    In the <b>"doNotWalk"</b> state, the walk light should never be on, whereas in the <b>"startWalking"</b> state, the walk light must be on and the request indicator off.</li>
    <li>The property <code>flashInterval</code> and is relevant solely to the <b>"finishWalking"</b> state, so to avoid confusion should not be included in the other states.</li>  
  </ol>
  ...
  <div class="breakout-box">
    ...
    <figure>
      <div>
        <img src={fooOrBarTypeNarrowingIfExample} alt="Foo or Bar Type Narrowing If example" style={{ paddingBottom: '58px' }} />
    ...
    <figure>
      <div>
        <img src={fooOrBarTypeNarrowingSwitchExample} alt="Foo or Bar Type Narrowing Switch example" style={{ paddingBottom: '58px' }} />
    ...
    This function is not never executed, and will throw an error if it is, but instead highlights regression errors in the code when changes erroneously allow unreachable code to be reachable.
    If a third type called <code>"baz"</code> were added to the state union without updating the switch statement, the following error would occur:</p> 

    <figure>
      <div>
        <img src={assertCannotReach} alt="Exhaustive switch with assert cannot reach error" style={{ paddingBottom: '39px' }} />
    ...
    <figure>
      <div>
        <img src={discriminatedUnionTypeNarrowing} alt="Type narrowing with FsmRx" style={{ paddingBottom: '39px' }} />
  ...
  <p>Discriminated unions are used to define different <code>stateData</code> variants for each of the states, as well as a default variant they all inherit from, in the following example:</p>
  ...
  ```
  <p>It is not strictly necessary for the base variant to extend <code>BaseStateData</code> however doing so provides added clarity and consistency.
  ...
  Including a base state in the discriminated union will break <b>TypeScripts</b> ability to perform type narrowing, as it will be unable to narrow beyond the base variant.</p> 

  <h3>Defining the Inputs and Outputs</h3> 
  
  <p>As per the requirements, upon pressing the request crossing button the <code>CrossingControlComponent</code> must emit a signal broadcasting that the interaction took place. 
  It then waits for a series of response signals that dictate the desired state it should transition to.
  This is achieved through the use of Angular <code>@Input</code> and <code>@Outputs</code> as follows:  </p>
  ...
  <div className="breakout-box">
    ...
    <p><code>UniqueDataService</code> is a small helper service used to create Universally Unique Identifiers (UUID) and unique data objects. 
  ...
  <p>The <code>CrossingControlComponent</code> contains a button element called <b>walk-button</b> which can be used to initiate the pedestrian crossing procedure. 
  Angular's binding syntax is employed to invoke the <code>handleWalkButtonClicked</code> event handler upon clicking this <b>walk-button</b>.</p>
  ...
  <p>As event handlers execute outside of the <b>FsmRx</b> context and can happen regardless of the current state, the <code>currentState$</code> getter is used to determine the state we are in.</p>
  ...
  <p>During the pedestrian crossing procedure it is vital that the states of the <code>CrossingControlComponent</code> and <code>TrafficLightRequestCrossingComponent</code> remain synchronized. 
  To ensure this happens, the <code>TrafficLightRequestCrossingComponent</code> acts as the "controller" with the responsibility of informing the "delegate" <code>CrossingControlComponent</code> of the state it should be in.
  This necessitates the addition of two composite states, <b>"stop_startWalking"</b> and <b>"stop_finishWalking"</b>, to the <code>TrafficLightRequestCrossingComponent</code> state string union.</p>

  ```ts
  type TrafficLightStates = "go" | "prepareToStop" | "stop" | "stop_startWalking" | "stop_finishWalking";
  ```
   
  <p>These composite states describe both the visual state of the <code>TrafficLightRequestCrossingComponent</code> (<b>"stop"</b>) and the instruction sent to the <code>CrossingControlComponent</code>.
  Entering <b>"stop_startWalking"</b> emits a signal telling the <code>CrossingControlComponent</code> to enter its <b>"startWalking"</b> state while entering <b>"stop_startWalking"</b> signals <b>"finishWalking"</b> and leaving <b>"stop_startWalking"</b> signals <b>"doNotWalk"</b>.</p>
  ...
  ```
  <p><i>Note: While <b>FsmRx</b> contains no formal method for defining composite states, in the following example a hyphen is used to indicate the hierarchical relationships between containing states and their substates.</i></p>

  <h3>Sequencing Events</h3>

  <p>When the <code>CrossingControlComponent</code> emits the <code>outputWalkButtonClicked</code> event, it is first captured by the <code>RequestCrossingAssemblyComponent</code> then passed to the <code>TrafficLightRequestCrossingComponent</code> via its <code>walkButtonEvent</code> <code>@Input</code>.
  ...
  ```
  <p><i><b>FsmRxComponents</b> implement their own <code>ngOnChanges</code> function for handling <code>fsmConfig</code> changes (see ["Part 5: Debugging FsmRxComponents using Storybook"](?path=/docs/5-debugging-fsmrxcomponents-using-storybook--docs)).
  When implementing custom <code>ngOnChanges</code> logic it is necessary to override this function and immediately call <code>super.ngOnChanges(changes)</code> to ensure default functionality is preserved.</i></p>


  <p>The <code>TrafficLightRequestCrossingComponent</code> schedules all state transitons using a function called <code>delayToStateChange</code>.
  ...
  <figure>
    <div>
      <img src={trafficLightRequestCrossingStateDiagram} alt="Traffic Light State Diagram" style={{ padding: '10px 20px 89px 10px', width: '470px' }} />
      <figcaption>The state diagram showing the branching behavior of the <code>TrafficLightRequestCrossingComponent</code> resulting from the checks perfomed when leaving the <b>"prepareToStop"</b> state.</figcaption>
    </div>
  </figure>
  
  <p>Eventhough the <code>pedestrianCrossingRequested</code> property is only evaluated during the <b>"prepareToStop"</b> state, it exists universally in the <code>stateData</code> definition of every state.
  ...
  ```
  <p>As previously discussed traversing the <b>"stop_startWalking"</b> and <b>"stop_finishWalking"</b> path will cause the <code>TrafficLightRequestCrossingComponent</code> to emit an event triggering the <code>CrossingControlComponent</code> to transition to the supplied state.
  As with previous events, this <code>outputCrossingControlBoxState</code> emission is first captured by the <code>RequestCrossingAssemblyComponent</code> before being relayed to the <code>CrossingControlComponent</code>. 
  ...
  <div class="breakout-box">
    ...
    <p>This article includes many state diagrams to help visualize the flow of an FSM. 
    While the generation of the SVGs from these diagrams is outside the scope of <b>FsmRx</b>, it does include functionality to output the string templates third-party libraries use to generate these diagrams.
    By default <b>FsmRx</b> outputs in <a href = "https://mermaid.js.org/" target="_blank">Mermaid.js</a> format which can be rendered in <b>Storybooks</b> using the <code>FsmRxDebugSetComponent</code> wrapper (see ["Part 5: Debugging FsmRxComponents using Storybook"](?path=/docs/5-debugging-fsmrxcomponents-using-storybook--docs)).</p>
    ...
    <figure>
      <div>
        <img src={trafficLightRequestCrossingStateDiagramExtract} alt="Extract from the TrafficLightRequestCrossing component state diagram" style={{ paddingBottom: '60px' }} />
  ...
  For this purpose <b>FsmRx</b> includes a <code>nextChangeStateTransition$</code> subject which emits when the state is changed and a <code>destroy$</code> subject which emits when an <b>FsmRx</b> FSM is destroyed. 
  For <code>FsmRxComponents</code> the <code>destroy$</code> subject is triggered automatically via the <code>ngOnDestroy</code> lifecycle hook but requires manual triggering for services and directives.</p>

  <p>The <code>interval</code> that handles the flashing of the crossing indicator light requires unsubscribing on both the destruction of the <code>CrossingControlComponent</code>, using <code>takeUntil(this.destroy$)</code>, and when transitioning to the next state, using <code>takeUntil(this.nextChangeStateTransition$)</code>.</p>
  ...
  By using transition cancelling these conditional transitions can be executed as part of a transition lifecycle hook. 
  Transition canceling takes advantage of <b>FsmRx's</b> self-recovery features to return to the last valid state when a lifecycle hook callback returns false.
  ...
  <div class="breakout-box examples">
    <p>The complete code for the following ["Traffic Light Request Crossing Alternate"](?path=/story/examples-5-traffic-light-request-crossing-alternate--default) <br/> example story can be found in the folder <br/><b>"/projects/examples/src/stories/traffic-light-request-crossing-alternate"</b>.</p>
  </div>

  <p>The <code>TrafficLightRequestCrossingAlternateComponent</code> defines condition transition logic using transition cancelling via the <code>prepareToStop</code> states <code>onLeave</code> callback.
  ...
  However, when true the transition is canceled and a new transition to the <b>"stop_startWalking"</b> state, with <code>pedestrianCrossingRequested</code> reset to false, is added to the queue for execution after the FSM has reset.</p>
  ...
  <p>Handling branching logic using transition canceling may help separate logical concerns, with the drawback of making transitions more difficult to follow.
