...
<article>
  ...
  The <b>NgxFsmRx</b> code repository contains two projects, the <b>NgxFsmRx</b> wrapper project and an <b>"Examples"</b> project which contains this documentation and its supporting code examples.</p> 
  ...
  <p>This <b>"Examples"</b> project takes advantage of <b>Storybook</b>, a front-end code playground for previewing and experimenting with front-end components.
  If you are not reading this documentation via the <b>Storybook</b> playground application It is strongly recommended that you run it using the following commands:</p> 
  ...
  <p><b>Storybook</b> provides a web interface where this documentation and the completed example components can be viewed. 
  In the vernacular of <b>Storybook</b>, these examples are presented as stories. 
  Stories allow us to set the inputs and default values of an <b>FsmRxComponent</b> through the use of <code>arg</code> controls, facilitating testing, experimentation and prototyping.</p>
  ...
  <div class="breakout-box examples">
    <p>The complete code for the following ["Traffic Light Simple"](?path=/story/examples-1-traffic-light-simple--default) <br/> example story can be found in the folder <br/><b>"/projects/examples/src/stories/traffic-light-simple"</b>.</p>
  </div>
 ...
 ```typescript
  ...
  <figure>
    <div>
      <img src={trafficLightSimpleStateDiagram} alt="Traffic Light State Diagram" style={{ padding: '10px 30px 100px 20px', width: '170px' }} />
  ...
  <p>These types can now be incorporated into the component class declaration by extending the abstract class <code>FsmRxComponent</code> and passing them in as type parameters.</p>
  ...
  <div className="breakout-box">
    ...
    <ol>
      ...
      <li><b>Storybook Integration </b>: <code>FsmRxComponent</code> exposes several functions that allow <b>Storybook</b> to display state diagrams and logging tools.
       If composition was used these functions would have to be manually exposed for every <code>FsmRxComponent</code> subclass.</li>
    </ol>
    <p>A compositional approach could be taken by extending <code>FsmRxInjectable</code> or <code>FsmRxComponent</code> into a service, but this has the potential to create more problems than it would solve.</p>
  </div>
  ...
  The <b>CSS</b> is outside the scope of this article but can be viewed in the <b>"Examples"</b> project of the <b>NgxFsmRx</b> repository.</p>
  ...
  If the transition is successful, the traffic light will advance to its <b>"go"</b> state and look like this: </p>
   ...
   <figure>
    <div>
      <img src={trafficLightSimpleGoState} alt="Traffic Light in Go State" style={{ paddingBottom: '40px' }} />
  ...
  <ul>
    ...
    This could involve activities like initialising timers, binding events, making API calls, or any other actions needed to prepare the new state for execution.</li>
  ...
  <p>These data types contain the hook they relate to and two properties to describe the before and after states of the transition. 
  ...
  <ul>
    ...
    <li><b>stateData</b>: The data relivant to this state.</li>
  ...
  <p>This gives us access to all the data required to control the flow of our FSM.</p>

  <p>These lifecycle hooks are specified within our <b>stateMap</b> and can be represented as either an inline function or a method call.</p>
  ...
  <p>The duration to wait in this state can be determined by consulting the <b>trafficLightTiming"</b> property of the state data. 
  As previously mentioned, this data can be retrieved from the <b>OnEnterStateChanges</b> argument either directly or via object destructuring</p> 
  ```ts
  onEnter: (onEnterStateChanges: OnEnterStateChanges<TrafficLightStates, "go", TrafficLightData, TrafficLightCanLeaveToMap>) => {
  ...
  Subscribing to this emission allows the <b>changeState</b> function to be called, advancing to the "prepareToStop" state after the specified delay.
  Given that all states share identical data, the spread operator can be used to supply this function with the current <b>stateData</b>, altering only the state property to reflect the desired next state.
  As the timer observable automatically completes after emitting, there is no need for an <b>onLeave</b> cleanup function to handle unsubscribing or any other cleanup operations.</p>
  ...
  <h3>Finializing the Simple Traffic Light Component</h3>
  ...
  <p>This can be achieved by updating the code to use a method call, rather than an inline function, and applying it to all of the states <b>OnEnter</b> hooks.
  ...
  ```
  <p>To successfully transition to the next state, we require two pieces of data: the current state we're in and our intended next state destination.
  The <code>enteringStateInfo</code> contains the properties <code>state</code> and <code>canLeaveTo</code>, which are the name of our impending new state and an array containing all of its possible future state transitions.
  As each state can only exit to one other state we can access the first entry in this array to get the name of our destination state.</p>
  ...
  ```
  <h3>Generating <code>FsmRxComponents</code> and <code>FsmRxServices</code> using Schematics</h3>

  <p>To aid in the creation of FSMs, <b>NgxFsmRx</b> contains schematics to automate the production of the previously described types and configuration objects for both <code>FsmRxComponents</code> and <code>FsmRxServices</code>. 
  ...
  <p>Both schematics guide you through the process of defining states, transition rules and lifecycle hooks, while also generating the necessary class, template and story files.</p> 
